...
const PI4 = StaticPolynomials.Polynomial(
      x1*x2*x3 + x1*x4*x5 + x2*x4*x6 + x2*x5*x6 )
...
function invariants_co(x::SVector{6, T}) where {T}
   ...
   I4 = x[1]*x[2]*x[3] + x[1]*x[4]*x[5] + x[2]*x[4]*x[6] + x[2]*x[5]*x[6]
...
function invariants_co_d(x::SVector{6, T}) where {T}
   ...
   o = @SVector ones(6)
   z = @SVector zeros(6)
   I2d = StaticPolynomials.gradient(PI2, x)
   I4d = StaticPolynomials.gradient(PI4, x)
